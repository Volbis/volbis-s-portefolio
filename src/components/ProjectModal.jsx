    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          exit={{ opacity: 0 }}
          onClick={onClose}
          className="fixed inset-0 bg-black/95 md:bg-black/80 md:backdrop-blur-sm z-[9999] flex items-center justify-center md:p-4"
        >
          {/* Desktop Modal */}
          <motion.div
            ...
            className="hidden md:block relative z-[10000] bg-gradient-to-br from-gray-900 to-black rounded-3xl shadow-2xl max-w-6xl w-full my-8 overflow-hidden border border-white/10"
          >
            {/* Bouton fermer */}
            <button
              onClick={onClose}
                ...
                <AnimatePresence mode="wait" custom={direction}>
                  ...
                  >
                    ...
                    <div className="mb-8">
                      <h3 className="text-white font-semibold text-sm mb-3 flex items-center gap-2 uppercase tracking-wide">
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-orange-400">
                          <path d="M12 20a8 8 0 1 0 0-16 8 8 0 0 0 0 16Z"/><path d="M12 14a2 2 0 1 0 0-4 2 2 0 0 0 0 4 Z"/><path d="M12 2v2"/><path d="M12 20v2"/><path d="m4.93 4.93 1.41 1.41"/><path d="m17.66 17.66 1.41 1.41"/><path d="M2 12h2"/><path d="M20 12h2"/><path d="m6.34 17.66-1.41 1.41"/><path d="m19.07 4.93-1.41 1.41"/>
                        </svg>
          ...
          <div className="md:hidden relative w-full h-full bg-black">
            ...
            <div className="h-full overflow-y-auto pt-16 pb-24" onClick={(e) => e.stopPropagation()}>
              <AnimatePresence mode="wait" custom={direction}>
                <motion.div
                  ...
                  transition={{ duration: 0.3, ease: "easeInOut" }}
                >
                  <div className="w-full min-h-[50vh] flex items-center justify-center bg-gradient-to-br from-gray-900 to-black p-4">
                    <ProjectMediaViewer project={project} />
                  </div>

                  <div className="p-6 bg-gradient-to-b from-black to-gray-950">
                    <div className="mb-6">
                      <h2 className="text-2xl font-bold text-white mb-3 bg-gradient-to-r from-orange-400 to-orange-600 bg-clip-text text-transparent">
                        {project.title}
                      </h2>
                    ...
                    <div className="mb-6">
                      <h3 className="text-white font-semibold text-xs mb-3 flex items-center gap-2 uppercase">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-orange-400">
                          <polyline points="16 18 22 12 16 6"/><polyline points="8 6 2 12 8 18"/>
                        </svg>
                        Technologies
                      </h3>
                      <div className="flex flex-wrap gap-2">
                        {project.tech.map((tech, i) => (
                          <span key={i} className="px-2.5 py-1 bg-orange-500/20 text-orange-300 text-xs rounded-full border border-orange-500/30">
                            {tech}
                          </span>
                        ))}
                      </div>
                      ...
                      <div className="space-y-2">
                        {project.features.map((feat, i) => (
                          <div key={i} className="flex items-start gap-2 text-gray-300 text-sm">
                            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-orange-400 mt-0.5">
                              <polyline points="20 6 9 17 4 12"/>
                            </svg>
                            <span>{feat}</span>
                          </div>
                    ...
                    <div className="flex flex-col gap-3">
                      {project.github && (
                        <a href={project.github} target="_blank" rel="noopener noreferrer" className="w-full px-4 py-3 bg-white/10 text-white rounded-xl border border-white/20 flex items-center justify-center gap-2 text-sm">
                      ...
                      {project.demo && (
                        <a href={project.demo} target="_blank" rel="noopener noreferrer" className="w-full px-4 py-3 bg-gradient-to-r from-orange-500 to-orange-600 text-white rounded-xl flex items-center justify-center gap-2 text-sm">
                          <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                            <path d="M20.2 6 3 11l-.9-2.4c-.3-1.1.3-2.2 1.3-2.5l13.5-4c1.1-.3 2.2.3 2.5 1.3Z"/>
                          </svg>
                          Voir la démo
            ...
            <div className="fixed bottom-0 left-0 right-0 z-50 bg-gradient-to-t from-black via-black/95 to-transparent p-4 pb-6" onClick={(e) => e.stopPropagation()}>
              <div className="flex gap-3">
                <motion.button 
                  ...
                  className="flex-1 px-4 py-3.5 bg-gradient-to-r from-white/5 to-white/10 backdrop-blur-sm text-white rounded-2xl border border-white/20 flex items-center justify-center gap-2 text-sm font-medium shadow-lg shadow-black/30"
                  whileTap={{ scale: 0.95 }}
                  whileHover={{ scale: 1.02 }}
                >
                ...
                <motion.button 
                  ...
                  className="flex-1 px-4 py-3.5 bg-gradient-to-r from-orange-500 to-orange-600 text-white rounded-2xl border border-orange-400/50 flex items-center justify-center gap-2 text-sm font-medium shadow-lg shadow-orange-500/40"
                  whileTap={{ scale: 0.95 }}
                  whileHover={{ scale: 1.02 }}
                >
                  Suivant
